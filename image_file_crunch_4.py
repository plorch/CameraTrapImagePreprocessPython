...
This version uses filepath rather than name and **does not use multiprocessing** 
(since this caused an error). Turns out, error is due to PIL issues on server. It works
...
folderpath = r"E:\UNPROCESSED\8th check August 2016\8thCheckAugust2016_1"
...
#   end with camera card name.
    ...
    # Use of dirpath here means their is only one "toupload" folder created per camera
                ...
                tt = time.clock()
                for path in newfilepaths:
                    fileResizeObscure(path)
