...
folderpath = r"P:\WildlifeCameraStudyImages\4thCheckApril2016\BR1157b"
...
# *** Note! These may need to change if we iterate over a higher level directory
    camname=os.path.split(folderpath)[1]
    check=os.path.split(os.path.split(folderpath)[0])[1]
    print "Check name: %s\n" % check
    ...
    for subdir, dirs, files in os.walk(folderpath):
        ...
        for dir in dirs:
            ...
            dirname = dir
            purge(os.path.join(subdir,dir),"\._")
            filepaths=get_image_paths(os.path.join(subdir,dir))
...
# After renaming get filepaths again        
            newfilepaths=get_image_paths(os.path.join(subdir,dir))
            #print "\n".join(newfilepaths)

# Use of subdir here should mean their is only one "toupload" folder created per subdir        
            if not os.path.exists(os.path.join(subdir,SAVE_DIRECTORY)):
                os.makedirs(os.path.join(subdir,SAVE_DIRECTORY))
        ...
        # Make manifest
            fpath=os.path.join(subdir,SAVE_DIRECTORY)
